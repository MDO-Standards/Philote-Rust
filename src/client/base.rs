...
impl DisciplineClient {
    ...
    pub async fn set_stream_options(&mut self, options: StreamOptions) -> Result<()> {
        ...
        self.client.set_stream_options(Request::new(proto_options)).await?;
        self.stream_options = options;
        Ok(())
    }
    ...
    pub async fn get_available_options(&mut self) -> Result<HashMap<String, String>> {
        ...
        for (option_name, data_type_int) in options_list.options.iter().zip(options_list.r#type.iter()) {
            let type_str = match data_type_int {
                0 => "bool",    // kBool
                1 => "int",     // kInt
                2 => "double",  // kDouble
                3 => "string",  // kString
                _ => return Err(PhiloteError::InvalidVariableType(format!("Unknown data type: {}", data_type_int))),
            };
    ...
    pub async fn set_options(&mut self, _options: HashMap<String, serde_json::Value>) -> Result<()> {
        ...
        self.client.set_options(Request::new(discipline_options)).await?;
        Ok(())
    }
    ...
    pub async fn get_variable_definitions(&mut self) -> Result<Vec<VariableMetaData>> {
        let response = self.client.get_variable_definitions(Request::new(())).await?;
    ...
    pub async fn get_partial_definitions(&mut self) -> Result<Vec<PartialsMetaData>> {
        let response = self.client.get_partial_definitions(Request::new(())).await?;
        let mut stream = response.into_inner();
