...
use crate::philote_info::{
    explicit_service_server::ExplicitService,
    Array, VariableType,
};
...
impl<D: ExplicitDiscipline + 'static> ExplicitService for ExplicitServer<D> {
    type ComputeFunctionStream = Pin<Box<dyn Stream<Item = std::result::Result<Array, Status>> + Send>>;
    ...
    type ComputeGradientStream = Pin<Box<dyn Stream<Item = std::result::Result<Array, Status>> + Send>>;
...
#[tonic::async_trait]
impl<D: ExplicitDiscipline + 'static> crate::philote_info::discipline_service_server::DisciplineService for ExplicitServer<D> {
    async fn get_info(
        &self,
        request: Request<()>,
