...
use crate::philote_info::{
    implicit_service_server::ImplicitService,
    Array, VariableType,
};
...
impl<D: ImplicitDiscipline + 'static> ImplicitServer<D> {
    ...
    async fn stream_arrays_as_chunks(
        ...
        let array_stream = chunks.into_iter().map(|chunk| {
            Ok(Array::from(chunk))
        });
    ...
    async fn stream_partials_as_chunks(
        ...
        for ((func_name, var_name), array) in partials {
            ...
            let array_chunks = crate::types::ArrayChunker::new(chunk_size)
                .chunk_array(func_name, &flat_data, VariableType::KPartial);
    ...
    async fn process_input_and_output_streams(
        ...
        // Preallocate arrays
        let (mut inputs, mut flat_inputs) = self.base.preallocate_inputs().await
            .map_err(|e| Status::internal(format!("Failed to preallocate inputs: {}", e)))?;
        
        let (mut outputs, mut flat_outputs) = self.base.preallocate_outputs().await
        ...
        self.base.process_input_stream(request_stream, &mut flat_inputs, Some(&mut flat_outputs)).await
...
impl<D: ImplicitDiscipline + 'static> ImplicitService for ImplicitServer<D> {
    type ComputeResidualsStream = Pin<Box<dyn Stream<Item = std::result::Result<Array, Status>> + Send>>;
    ...
    async fn compute_residuals(
        ...
        let residuals = discipline.compute_residuals(&inputs, &outputs).await
            .map_err(|e| Status::internal(format!("Compute residuals failed: {}", e)))?;
        drop(discipline);
        
        // Stream residuals back to client
        let residual_stream = self.stream_arrays_as_chunks(&residuals, VariableType::KResidual).await;
        
        Ok(Response::new(residual_stream))
    }
    
    type SolveResidualsStream = Pin<Box<dyn Stream<Item = std::result::Result<Array, Status>> + Send>>;
    ...
    async fn solve_residuals(
        ...
        let input_stream = request.into_inner();
        self.base.process_input_stream(input_stream, &mut flat_inputs, None).await
            .map_err(|e| Status::internal(format!("Failed to process input stream: {}", e)))?;
        ...
        let discipline = self.base.discipline().read().await;
        let outputs = discipline.solve_residuals(&inputs).await
            .map_err(|e| Status::internal(format!("Solve residuals failed: {}", e)))?;
        drop(discipline);
        
        // Stream outputs back to client
        let output_stream = self.stream_arrays_as_chunks(&outputs, VariableType::KOutput).await;
        
        Ok(Response::new(output_stream))
    }
    
    type ComputeResidualGradientsStream = Pin<Box<dyn Stream<Item = std::result::Result<Array, Status>> + Send>>;
    ...
    async fn compute_residual_gradients(
        ...
        let discipline = self.base.discipline().read().await;
        let partials = discipline.residual_partials(&inputs, &outputs).await
...
#[tonic::async_trait]
impl<D: ImplicitDiscipline + 'static> crate::philote_info::discipline_service_server::DisciplineService for ImplicitServer<D> {
    async fn get_info(
        &self,
        request: Request<()>,
