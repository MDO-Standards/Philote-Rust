    ...
    match client.get_partial_definitions().await {
        Ok(partials) => {
            ...
            for partial in partials {
                println!("   - {}/{}: {:?}", partial.name, partial.subname, partial.shape);
            }
        },
        Err(e) => {
...
pub async fn start_test_server() -> Result<()> {
    ...
    impl Discipline for TestParaboloid {
        fn name(&self) -> &str { "TestParaboloid" }
        fn version(&self) -> &str { "1.0.0" }
        fn is_continuous(&self) -> bool { true }
        fn is_differentiable(&self) -> bool { true }
        ...
        fn add_option(&mut self, name: &str, option_type: &str) -> Result<()> {
            self.options.insert(name.to_string(), option_type.to_string());
            Ok(())
        }
    ...
    impl ExplicitDiscipline for TestParaboloid {
        async fn compute(&self, inputs: &ArrayMap) -> Result<ArrayMap> {
            let x = inputs.get("x").ok_or_else(|| PhiloteError::VariableNotFound("x".to_string()))?;
            let y = inputs.get("y").ok_or_else(|| PhiloteError::VariableNotFound("y".to_string()))?;
        ...
        async fn compute_partials(&self, inputs: &ArrayMap) -> Result<philote::PartialMap> {
            ...
            partials.insert(("f".to_string(), "x".to_string()), ArrayD::from_elem(vec![1], df_dx));
            partials.insert(("f".to_string(), "y".to_string()), ArrayD::from_elem(vec![1], df_dy));
            Ok(partials)
        }
    }
    ...
    Server::builder()
        .add_service(philote::philote_info::explicit_service_server::ExplicitServiceServer::new(server_impl))
        .serve(addr)
        .await
